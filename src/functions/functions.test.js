...
const response = {status: 'correct', calculation: '50'}
// Bad conversion
const params_conversion = ['10','c','60','f']
const response_conversion = {status: 'incorrect', calculation: '50'}
// Missing parameters
const params_missing_1 = ['10','c','60']
const params_missing_2 = ['10','c']
const params_missing_3 = ['10']
const params_missing_4 = []
const response_missing = {status: 'invalid', calculation: null}
// Letter in startingValue parameter
const params_letters = ['987.g','c','78','f']
const response_letters = {status: 'invalid', calculation: null}
// Letter in convertedValue parameter
const params_letters_converted = ['10','c','a','f']
const response_letters_converted = {status: 'incorrect', calculation: '50'}

describe('The conversion functions', () => {
  test('return success and proper conversion with correct parameters', () => {
    expect(performConversion(...params)).toEqual(response);
  });
  test('to return the proper conversion when the student is incorrect', () => {
    expect(performConversion(...params_conversion)).toEqual(response_conversion);
  });
  test('to return invalid with one missing parameter', () => {
    expect(performConversion(...params_missing_1)).toEqual(response_missing);
  });
  test('to return invalid with two missing parameters', () => {
    expect(performConversion(...params_missing_2)).toEqual(response_missing);
  });
  test('to return invalid with three missing parameters', () => {
    expect(performConversion(...params_missing_3)).toEqual(response_missing);
  });
  test('to return invalid with four missing parameters', () => {
    expect(performConversion(...params_missing_4)).toEqual(response_missing);
  });
  test('to return invalid if letters are used in the starting value', () => {
    expect(performConversion(...params_letters)).toEqual(response_letters);
  });
  test('to return incorrect if letters are used in the converted value', () => {
    expect(performConversion(...params_letters_converted)).toEqual(response_letters_converted);
  });
});